...
const int calibrate = 20; // calibrated over 100 averages

int sensorPins[numSensors] = {A0,A1}; // Example sensor pins
int outputPins[numSensors] = {23,20}; // Example output pins
...
void loop() {
  ...
  if (buttonStateG1 == HIGH && buttonG1Pressed == false && buttonG2Pressed == false) {
    ...
    startTime = millis();
    randomSeed(analogRead(A3)); // Use an analog pin to seed the random number generator
