...
const int beginButton = 10; // Change to buttom pin
const int piezo = 7; // Change to buzzer pin
int currentMillis = 0;

// Define state variables
bool buttonPressed = false;

const int numSensors = 2; //Change to number of sensor inputs
const int numSamples = 5; // rolling average over every 5 values
const int calibrate = 20; // Can change to calibrate over different amount of averages

// Define sensor and output pins and thresholds for each sensor
int sensorPins[numSensors] = {A0,A1}; // Change to sensor pins
int outputPins[numSensors] = {23,20}; // Change to output pins
int rollingSensorVal[numSensors][numSamples];
int sensorIndex[numSensors] = {0};
int thresholds[numSensors] = {250,250}; // Change!|whatever value - calibration value [i]| >= thresholds[i] so plug in the DIFFERENCE needed to trigger
...
void setup() {
  ...
  lcd.setCursor(0, 1);
  lcd.print("Press to begin game 2->");
}
