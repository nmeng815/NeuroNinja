...
bool wrong = false; //increase duration if a mistake is made only once
...
void loop() {
  ...
  if (buttonPressed == true){
    ...
    if (duration <= minDuration){
      ...
      if (wrong == true){
      duration = 6000 - score*100 + 500;
      wrong = false;
      } else{
        duration = 6000 - score*100;
      } //fix mistake to only add .5 when the mistake is made not every time
    }
...
void resetForNextRound() {
    delay(duration - currentMillis % duration);
    startTime = millis();
